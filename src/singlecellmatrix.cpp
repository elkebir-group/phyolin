...
SingleCellMatrix::SingleCellMatrix(std::vector<std::vector<int> > sscm)
    : _cells(sscm.size()), _sites(sscm[0].size()), _sscm(), _cellIDs(), _mutIDs()
{
...
SingleCellMatrix::SingleCellMatrix(std::string filename, std::string sep, bool header) : _sscm()
{
